...
if(isset($_POST["submit"]) && !empty($_FILES["file"]["name"])){
    ...
    $subject = "EX-QUIZ_ME EVENT-ECENTRA 2020";
    $message1 = "Thank You $name1 For Registering With Ecentra2020. Your Payment Details Will Be Updated Soon";
    $message4 = "A New User Has Been Registered To Ex-Quiz-Me EVENT.Check The Database and Verify The Details";
    ...
    if (!file_exists($targetFilePath)) {
        if(in_array($fileType, $allowTypes)){
            ...
            if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
                ...
                $insert = $db->query("INSERT into qzevent (name1, id1, email1, phone1, college1, branch1,acc,payment, uploaded_on,ecode)
                 VALUES ('$name1', '$id1','$email1', '$phone1','$college1', '$branch1','$acc','".$fileName."', NOW(),'xyz')");
                $insert = $db->query("UPDATE qzevent SET ecode = concat( ecentraid, id )");
                if($insert){
                    $statusMsg = "The file <b>".$fileName. "</b> has been uploaded successfully." . $backlink;
                    header("location:https://ecentra2020.in/image-upload/successfulqz.php");
                }else{
