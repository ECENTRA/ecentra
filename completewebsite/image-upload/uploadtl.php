...
if(isset($_POST["submit"]) && !empty($_FILES["file"]["name"])){
    ...
    $message4 = "A New User Has Been Registered To Tech-Ladders EVENT.Check The Database and Verify The Details";
    ...
    if (!file_exists($targetFilePath)) {
        if(in_array($fileType, $allowTypes)){
            ...
            if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
                ...
                $insert = $db->query("INSERT into tlevent (name1, id1, email1, phone1, college1, branch1,acc,payment, uploaded_on,ecode)
                 VALUES ('$name1', '$id1','$email1', '$phone1','$college1', '$branch1','$acc','".$fileName."', NOW(),'xyz')");
                $insert = $db->query("UPDATE tlevent SET ecode = concat( ecentraid, id )");
                if($insert){
                    $statusMsg = "The file <b>".$fileName. "</b> has been uploaded successfully." . $backlink;
                    header("location:https://ecentra2020.in/image-upload/successfultl.php");
                }else{
